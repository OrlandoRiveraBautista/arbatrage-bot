...
// One Eth in wei for later conversions
const ONE_ETH = web3.utils.toWei("1", "Ether");

// Minimum eth to swap
const ETH_TO_TRADE_IN_WEI = web3.utils.toWei(".001", "Ether"); // .01 * 10^18 wei : 10_000_000_000_000_000
console.log("Eth Amount", ETH_TO_TRADE_IN_WEI);

// Price of Eth when we want to sell
const ETH_SELL_PRICE_IN_WEI = web3.utils.toWei("200", "Ether"); // 384 * 10^18

// 384.637114872807237537
// 384.637114872807237537

/** ---------------------------------- WE LEFT OFF HERE --------------------------------------- */
async function sellEth(ethToSellInWei, currentEthPriceInWei) {
  ...
  const SETTINGS = {
    ...
    value: ethToSellInWei, // Amount of Ether to Swap
  };

  // Perform Swap
  console.log("Performing swap...");
  let result = await exchangeContract.methods
    .ethToTokenSwapInput(currentEthPriceInWei.toString(), DEADLINE)
    .send(SETTINGS);
  console.log(
...
async function monitorPrice() {
  ...
  try {
    // Getting current Eth price in wei
    const ETH_PRICE_IN_WEI = await exchangeContract.methods
      .getEthToTokenInputPrice(ONE_ETH)
      .call();
    // Converting Eth price from wei to DAI
    const daiAmount = web3.utils.fromWei(daiAmount.toString(), "Ether");
    console.log("Eth Price:", daiAmount, price, ETH_SELL_PRICE_IN_WEI, " DAI");
    // Eth Price: 384637114872807237537 384.637114872807237537 200000000000000000000  DAI

    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt
    /**
     *  When comparing prices use BigInt to get the most accurate representation of a value.
     */

    // Check to see if the current eth price is smaller than our sell price
    if (BigInt(ETH_PRICE_IN_WEI) <= BigInt(ETH_SELL_PRICE_IN_WEI)) {
      // BigInt
      ...
      // Sell Eth
      await sellEth(ETH_TO_TRADE_IN_WEI, ETH_PRICE_IN_WEI);
